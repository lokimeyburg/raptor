      ...
      context 'and bogus authentication credentials' do
        it 'sends back an error message' do
          messages  = em_stream do |websocket, messages|
            ...
            when 1
              websocket.send({ event: 'raptor:subscribe', data: { channel: 'presence-channel', auth: 'bogus' } }.to_json)
            else
              EM.stop
            end
          end
    ...
    context 'subscribing with channel data' do
      context 'and bogus authentication credentials' do
        it 'sends back an error message' do
          ...
          messages.should have_attributes first_event: 'raptor:connection_established', count: 2,
            id_present: true

          # Channel id should be in the payload
          messages.last['event'].should == 'raptor:error'
      ...
      context 'with genuine authentication credentials'  do
        it 'sends back a success message' do
          ...
          messages.last.should == {"channel"=>"presence-channel",
                                   "event"=>"raptor_internal:subscription_succeeded",
                                   "data"=>{"presence"=>
                                            {"count"=>1,
        ...
        context 'with more than one subscriber subscribed to the channel' do
          it 'sends a member added message to the existing subscribers' do
            ...
            # Channel id should be in the payload
            messages[1].should == {"channel"=>"presence-channel", "event"=>"raptor_internal:subscription_succeeded",
                                     "data"=>{"presence"=>{"count"=>1, "ids"=>["0f177369a3b71275d25ab1b44db9f95f"], "hash"=>{"0f177369a3b71275d25ab1b44db9f95f"=>{"name"=>"SG"}}}}}

            messages.last.should == {"channel"=>"presence-channel", "event"=>"raptor_internal:member_added",
          ...
          it 'does not send multiple member added and member removed messages if one subscriber opens multiple connections, i.e. multiple browser tabs.' do
            ...
            messages.one? { |message| message['event'] == 'raptor_internal:member_added'   && message['data']['user_id'] == '37960509766262569d504f02a0ee986d' }.should be_true
            messages.one? { |message| message['event'] == 'raptor_internal:member_removed' && message['data']['user_id'] == '37960509766262569d504f02a0ee986d' }.should be_true
