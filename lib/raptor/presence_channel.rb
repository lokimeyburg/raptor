...
module Raptor
  class PresenceChannel < Channel
    ...
    def initialize(attrs)
      ...
      Raptor::Redis.subscribe 'raptor:connection_notification'
    end
    ...
    def get_roster
      ...
      Fiber.new do
        f = Fiber.current
        Raptor::Redis.hgetall(channel_id).
    ...
    def roster_add(key, value)
      ...
      Raptor::Redis.hset(channel_id, key, value)
    end

    def roster_remove(key)
      # Remove subscription info from Redis.
      Raptor::Redis.hdel(channel_id, key)
    end
    ...
    def publish_connection_notification(payload, retry_count=0)
      ...
      # channel.
      Raptor::Redis.publish('raptor:connection_notification', payload.to_json).
