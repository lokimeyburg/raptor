...
module Raptor
  class Channel
    ...
    def initialize(attrs)
      super
      Raptor::Redis.subscribe channel_id
    end
    ...
    def subscribe *a, &blk
      Raptor::Redis.hincrby('channel_subscriber_count', channel_id, 1).
        callback do |value|
          Raptor::Webhook.post name: 'channel_occupied', channel: channel_id if value == 1
        end
    ...
    def unsubscribe *a, &blk
      Raptor::Redis.hincrby('channel_subscriber_count', channel_id, -1).
        callback do |value|
          Raptor::Webhook.post name: 'channel_vacated', channel: channel_id if value == 0
        end
    ...
    def send_client_message(message)
      Raptor::Redis.publish(message['channel'], message.to_json) if authenticated?
    end
