...
# RaptorServer channel. Relay events received from Redis into the
...
module RaptorServer
  class PresenceChannel < Channel
    ...
    def get_roster
      ...
      Fiber.new do
        f = Fiber.current
        RaptorServer::Redis.hgetall(channel_id).
    ...
    def roster_add(key, value)
      ...
      RaptorServer::Redis.hset(channel_id, key, value)
    end

    def roster_remove(key)
      # Remove subscription info from Redis.
      RaptorServer::Redis.hdel(channel_id, key)
    end
