...
* Raptor.js
* 
* This file is based off of an earlier version of Raptor's client-side code, except that this uses the
* Raptor "url-like" channel syntax, there's no need to specify your app_id and this does not support 
* any type of fallback like long-polling or Flash.
*/
...
;(function() {
  ...
  var Raptor = function(app_key, options) {
    this.options = options || {};
    this.key = app_key;
    this.channels = new Raptor.Channels();
    this.global_emitter = new Raptor.EventsDispatcher()

    var self = this;

    this.checkAppKey();

    this.connection = new Raptor.Connection(this.key, this.options);
    ...
    this.connection
      ...
      .bind('message', function(params) {
        var internal = (params.event.indexOf('raptor_internal:') === 0);
        if (params.channel) {
          var channel;
      ...
      .bind('error', function(err) {
        Raptor.warn('Error', err);
      });

    Raptor.instances.push(this);

    if (Raptor.isReady) self.connect();
  };
  Raptor.instances = [];
  Raptor.prototype = {
    ...
    subscribe: function(channel_name) {
      ...
      if (this.connection.state === 'connected') {
        channel.authorize(this.connection.socket_id, this.options, function(err, data) {
          if (err) {
            channel.emit('raptor:subscription_error', data);
          } else {
            self.send_event('raptor:subscribe', {
    ...
    unsubscribe: function(channel_name) {
      ...
      if (this.connection.state === 'connected') {
        this.send_event('raptor:unsubscribe', {
          channel: channel_name
        });
      }
    },
    ...
    checkAppKey: function() {
      if(this.key === null || this.key === undefined) {
        Raptor.warn('Warning', 'You must pass your app key when you instantiate Raptor.');
      }
    }
  };
  ...
  // To receive log output provide a Raptor.log function, for example
  // Raptor.log = function(m){console.log(m)}
  Raptor.debug = function() {
    if (!Raptor.log) return
    Raptor.log(Raptor.Util.stringify.apply(this, arguments))
  }
  Raptor.warn = function() {
    if (window.console && window.console.warn) {
      window.console.warn(Raptor.Util.stringify.apply(this, arguments));
    } else {
      if (!Raptor.log) return
      Raptor.log(Raptor.Util.stringify.apply(this, arguments));
    }
  ...
  // Raptor defaults
  Raptor.VERSION = '1.12.5';
  // WS connection parameters
  Raptor.host = '0.0.0.0';
  Raptor.ws_port = 8080;
  Raptor.wss_port = 8080;
  // Other settings
  Raptor.channel_auth_endpoint = '/raptor/auth';
  Raptor.activity_timeout = 120000;
  Raptor.pong_timeout = 30000;

  Raptor.isReady = false;
  Raptor.ready = function() {
    Raptor.isReady = true;
    for (var i = 0, l = Raptor.instances.length; i < l; i++) {
      Raptor.instances[i].connect();
    }
  };
...
/* TODO: remove this event emmiter and use the "wild-card eventEmitter" that I'm using in the prototype for webrtc.

Abstract event binding
Example:

    var MyEventEmitter = function(){};
    MyEventEmitter.prototype = new Raptor.EventsDispatcher;
...
--------------------------------------------------------*/
  ...
  CallbackRegistry.prototype.remove = function(eventName, callback) {
    if(this.get(eventName)) {
      var index = Raptor.Util.arrayIndexOf(this.get(eventName), callback);
  ...
  this.Raptor.EventsDispatcher = EventsDispatcher;
}).call(this);
...
;(function() {
  ...
  function Machine(initialState, transitions, stateActions) {
    Raptor.EventsDispatcher.call(this);
  ...
  Machine.prototype.transition = function(nextState, data) {
    ...
    if (prevState && (Raptor.Util.arrayIndexOf(this.transitions[prevState], nextState) == -1)) {
  ...
  Raptor.Util.extend(Machine.prototype, Raptor.EventsDispatcher.prototype);

  this.Raptor.Machine = Machine;
}).call(this);

;(function() {
  /*
    Interface with window.navigator.onLine,
  ...
  */
  ...
  Raptor.Util.extend(NetInfo.prototype, Raptor.EventsDispatcher.prototype);

  this.Raptor.NetInfo = NetInfo;
}).call(this);
...
;(function() {
  ...
  function Connection(key, options) {
    ...
    this.ping = true
    this.options = Raptor.Util.extend({encrypted: false}, options);

    this.netInfo = new Raptor.NetInfo();
    ...
    this._machine = new Raptor.Machine('initialized', machineTransitions, {
      ...
      connectingPre: function() {
        ...
        Raptor.debug('Connecting', url);
        self.socket = new Raptor.Transport(url);
      ...
      openPre: function() {
        ...
        // allow time to get connected-to-Raptor message, otherwise close socket, try again
      ...
      failedPre: function() {
        updateState('failed');
        Raptor.debug('WebSockets are not available in this browser.');
      },
    ...
    function connectBaseURL(isSecure) {
      ...
      var port = ssl ? Raptor.wss_port : Raptor.ws_port;
      var scheme = ssl ? 'wss://' : 'ws://';

      return scheme + Raptor.host + ':' + port;
    }

    function connectPath(key) {
      var path = '/app/' + key + '?protocol=5&client=js&version=' + Raptor.VERSION;
      return path;
    }
    ...
    function resetActivityCheck() {
      ...
      if (self.ping) {
        self._activityTimer = setTimeout(function() {
          self.send_event('raptor:ping', {})
          // Wait for pong response
          self._activityTimer = setTimeout(function() {
            self.socket.close();
          }, (self.options.pong_timeout || Raptor.pong_timeout))
        }, (self.options.activity_timeout || Raptor.activity_timeout))
      }
    }
    ...
    function handleCloseCode(code, message) {
      ...
      self.emit('error', {type: 'RaptorError', data: {code: code, message: message}});
    ...
    function ws_onMessageOpen(event) {
      ...
      if (params !== undefined) {
        if (params.event === 'raptor:connection_established') {
          self._machine.transition('connected', params.data.socket_id);
        } else if (params.event === 'raptor:error') {
    ...
    function ws_onMessageConnected(event) {
      ...
      var params = parseWebSocketEvent(event);
      if (params !== undefined) {
        Raptor.debug('Event recd', params);

        switch (params.event) {
          case 'raptor:error':
            self.emit('error', {type: 'RaptorError', data: params.data});
            break;
          case 'raptor:ping':
            self.send_event('raptor:pong', {})
            break;
        }
    ...
    **/
    ...
    function updateState(newState, data) {
      ...
      if (prevState !== newState) {
        Raptor.debug('State changed', prevState + ' -> ' + newState);
  ...
  Connection.prototype.connect = function() {
    // no WebSockets
    if (!this._machine.is('failed') && !Raptor.Transport) {
  ...
  Connection.prototype.send_event = function(event_name, data, channel) {
    ...
    Raptor.debug('Event sent', payload);
  ...
  Raptor.Util.extend(Connection.prototype, Raptor.EventsDispatcher.prototype);
  this.Raptor.Connection = Connection;
}).call(this);

;(function() {
  Raptor.Channels = function() {
    this.channels = {};
  };

  Raptor.Channels.prototype = {
    add: function(channel_name, raptor) {
      var existing_channel = this.find(channel_name);
      if (!existing_channel) {
        var channel = Raptor.Channel.factory(channel_name, raptor);
  ...
  Raptor.Channel = function(channel_name, raptor) {
    var self = this;
    Raptor.EventsDispatcher.call(this, function(event_name, event_data) {
      Raptor.debug('No callbacks on ' + channel_name + ' for ' + event_name);
    });
    ...
    this.bind('raptor_internal:subscription_succeeded', function(data) {
      self.onSubscriptionSucceeded(data);
    });
  };

  Raptor.Channel.prototype = {
    // inheritable constructor
    init: function() {},
    disconnect: function() {
      this.subscribed = false;
      this.emit("raptor_internal:disconnected");
    },

    onSubscriptionSucceeded: function(data) {
      this.subscribed = true;
      this.emit('raptor:subscription_succeeded');
    },
    ...
    trigger: function(event, data) {
      return this.raptor.send_event(event, data, this.name);
    }
  };

  Raptor.Util.extend(Raptor.Channel.prototype, Raptor.EventsDispatcher.prototype);

  Raptor.Channel.PrivateChannel = {
    authorize: function(socketId, options, callback){
      var self = this;
      var authorizer = new Raptor.Channel.Authorizer(this, 'ajax', options);
      return authorizer.authorize(socketId, function(err, authData) {
        if(!err) {
          self.emit('raptor_internal:authorized', authData);
        }
  ...
  Raptor.Channel.PresenceChannel = {
    init: function(){
    ...
    onSubscriptionSucceeded: function(data) {
      ...
      // emitting the raptor:subscription_succeeded.  It will do this after it has done its work.
    }
  };
  ...
  var Members = function(channel) {
    ...
    channel.bind('raptor_internal:authorized', function(authorizedData) {
      var channelData = JSON.parse(authorizedData.channel_data);
      channel.bind("raptor_internal:subscription_succeeded", function(subscriptionData) {
        self._members_map = subscriptionData.presence.hash;
        self.count = subscriptionData.presence.count;
        self.me = self.get(channelData.user_id);
        channel.emit('raptor:subscription_succeeded', self);
      });
    });

    channel.bind('raptor_internal:member_added', function(data) {
      if(self.get(data.user_id) === null) { // only incr if user_id does not already exist
        self.count++;
      }

      self._members_map[data.user_id] = data.user_info;
      channel.emit('raptor:member_added', self.get(data.user_id));
    });

    channel.bind('raptor_internal:member_removed', function(data) {
      var member = self.get(data.user_id);
      if(member) {
        delete self._members_map[data.user_id];
        self.count--;
        channel.emit('raptor:member_removed', member);
      }
    });

    channel.bind('raptor_internal:disconnected', function() {
      reset.call(self);
    });
  };
  ...
  Raptor.Channel.factory = function(channel_name, raptor){
    var channel = new Raptor.Channel(channel_name, raptor);
    if (channel_name.indexOf('private-') === 0) {
      Raptor.Util.extend(channel, Raptor.Channel.PrivateChannel);
    } else if (channel_name.indexOf('presence-') === 0) {
      Raptor.Util.extend(channel, Raptor.Channel.PrivateChannel);
      Raptor.Util.extend(channel, Raptor.Channel.PresenceChannel);
    };
    channel.init();
    return channel;
  };
...
;(function() {
  Raptor.Channel.Authorizer = function(channel, type, options) {
  ...
  Raptor.Channel.Authorizer.prototype = {
    ...
    authorize: function(socketId, callback) {
      return Raptor.authorizers[this.type].call(this, socketId, callback);
    }
  };


  Raptor.auth_callbacks = {};
  // Only one type of authorizer for now (ajax).
  Raptor.authorizers = {
    ajax: function(socketId, callback){
      ...
      xhr.open("POST", Raptor.channel_auth_endpoint, true);
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
          if (xhr.status == 200) {
            ...
            Raptor.warn("Couldn't get auth info from your webapp", xhr.status);
...
;(function() {
  ...
  if (window['WebSocket']) {
    Raptor.Transport = window['WebSocket'];
    Raptor.TransportType = 'native';
  }
